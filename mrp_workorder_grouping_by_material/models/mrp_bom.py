...
class MrpBom(models.Model):
    ...
    def _check_one_main_material(self):
        for record in self:
            operations = []
            for line in record.bom_line_ids.filtered(lambda x: x.main_material):
                ...
                if operation_id not in operations:
                    ...
                    raise exceptions.ValidationError(
                        _(
                            ...
                            "Check lines that contains operation: {}"
                        ).format(line.operation_id.name)
                    )
    ...
    def _check_main_material_operation(self):
        for record in self:
            if record.main_material and not record.operation_id:
                raise exceptions.ValidationError(
                    _("Main material must have an operation")
                )
