...
class MrpWorkorderNest(models.Model):
    ...
    done_cancel_lines = fields.Boolean(
        string="No Active Lines", compute="_compute_active_lines", store=True
    )
    date_planned_start = fields.Datetime(
        compute="_compute_date_planned_start", string="Scheduled Date Start", store=True
    )
    show_worksheet = fields.Boolean(compute="_compute_show_worksheet")

    @api.depends("nested_line_ids", "nested_line_ids.date_planned_start")
    def _compute_date_planned_start(self):
        for nest in self:
            dates = list(
                filter(lambda x: x, nest.mapped("nested_line_ids.date_planned_start"))
            )
            if dates:
    ...
    def _compute_qty_producing(self):
        for nest in self:
            nest.qty_producing = sum(
                nest.nested_line_ids.filtered(lambda x: x.state == "progress").mapped(
                    "qty_producing"
                )
            )
    ...
    def name_get(self):
        ...
        for record in self:
            name = "{}{}{}".format(
                record.name or "", record.code and "/" or "", record.code or ""
            )
    ...
    def _compute_line_states(self):
        for nest in self:
            ...
            if nest.line_working_state == "blocked" and nest.state not in [
                "done",
                "blocked",
            ]:
                nest.pre_block_state = nest.state
