...
class NestedNewLine(models.TransientModel):
    ...
    possible_product_ids = fields.Many2many(
        comodel_name="product.product", compute="_compute_possible_products"
    )
    line_ids = fields.One2many(
        comodel_name="nested.lines.info", inverse_name="nest_id", string="Nest Lines"
    )
    ...
    def _prepare_lines(self, product=False):
        ...
        if self.env.context.get("active_model") == "mrp.workorder":
            ...
            if product:
                workorders = workorders.filtered(lambda w: w.product_id == product)
    ...
    @api.depends(
        "line_ids", "line_ids.workorder_id", "line_ids.workorder_id.product_id"
    )
    ...
    def _onchange_product_id(self):
        ...
        self.filtered_line_ids = lines.filtered(
            lambda fnl: fnl.product_id == self.product_id
        )
        ...
        for line in lines.filtered("qty_producing"):
            ...
            if line.qty_producing + wo.qty_produced > wo.qty_production:
                raise exceptions.ValidationError(
                    _(
                        "the quantity to be produced must be less than or "
                        "equal to quantity produced"
                    )
                )
            workcenter_dict = main_product_workcenter.get(wo.main_product_id.id)
            if workcenter_dict:
...
class NestedLinesInfo(models.TransientModel):
    ...
    nest_id = fields.Many2one(
        comodel_name="nested.new.line", string="Nest", required=True
    )
    workorder_id = fields.Many2one(
        comodel_name="mrp.workorder", string="Workorder", required=True
    )
    product_id = fields.Many2one(comodel_name="product.product")
    qty_producing = fields.Float(string="Quantity to Nest")
    ...
    qty_nested = fields.Float(string="Already Nested Quantity")
    # finished_qty_nested = fields.Float(
    #     string="Finished Nested Quantity")
    qty_nested_remaining = fields.Float(string="Quantity To Be Nested")
    qty_remaining = fields.Float(
        string="Quantity To Be Produced", digits="Product Unit of Measure"
    )
