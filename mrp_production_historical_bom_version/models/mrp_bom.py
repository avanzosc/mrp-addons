...
class MrpBom(models.Model):
    ...
    def _copy_bom(self):
        new_bom = super(MrpBom, self)._copy_bom()
        new_bom._create_version_activate_historical("version")
        return new_bom

    def button_activate(self):
        result = super(MrpBom, self).button_activate()
        for bom in self:
    ...
    def _create_version_activate_historical(self, text):
        ...
        if text == "version":
            my_text = _(u"It has been changed to version {}.").format(
                self.version)
        else:
            my_text = _(u"The BoM with version {}, has been activated.").format(
                self.version)
        vals = {"bom_id": self.id,
