    ...
    def post_inventory(self):
        for order in self:
            ...
            for moveline in moves_to_finish.mapped("move_line_ids"):
                if (
                    moveline.move_id.has_tracking != "none"
                    and moveline.product_id == order.product_id
                    or moveline.lot_id in consume_move_lines.mapped("lot_produced_ids")
                ):
                    if any([not ml.lot_produced_ids for ml in consume_move_lines]):
                        raise UserError(
                            _(
                                "You can not consume without telling for which "
                                "lot you consumed it"
                            )
                        )
