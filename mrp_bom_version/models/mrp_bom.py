...
class MrpBom(models.Model):
    ...
    active = fields.Boolean(
        default=_default_active, readonly=True, copy=False,
        states={"draft": [("readonly", False)]}
    )
    historical_date = fields.Date(
        string="Historical Date", readonly=True, copy=False
    )
    state = fields.Selection(
        ...
        string="State", index=True, readonly=True, default=_default_state,
    ...
    version = fields.Integer(
        string="Version", states={"historical": [("readonly", True)]},
    ...
    num_old_versions = fields.Integer(
        string="Num. Old Versions", comodel_name="mrp.bom",
    ...
    def button_draft(self):
        bom_active_draft = self.env["ir.config_parameter"].sudo().get_param(
            "mrp_bom_version.bom_active_draft", False)
        self.write({
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        """
        search_state = False
        if ("state" in self.env.context and
                self.env.context.get("state", False)):
    ...
    def _bom_find(self, products, picking_type=None, company_id=False,
        ...
        bom = super(MrpBom, self.with_context(state="active"))._bom_find(
            products=products, picking_type=picking_type, company_id=company_id,
            bom_type=bom_type)
        return bom
