...
class MrpProductionHistorical(models.Model):
    ...
    def _send_add_email(self):
        template = self.env.ref(
            "mrp_production_historical_follower.mrp_production_historical_add", False
        )
        if template:
            for partner in self.production_id.mrp_production_follower_ids:
                self.send_email_to_follower(template, partner)
    ...
    def _send_ldm_email(self):
        template = self.env.ref(
            "mrp_production_historical_follower.mrp_production_histo_bom1", False
        )
        if template:
            for partner in self.production_id.mrp_production_follower_ids:
                self.send_email_to_follower(template, partner)
    ...
    def _send_ldm2_email(self):
        template = self.env.ref(
            "mrp_production_historical_follower.mrp_production_histo_bom2", False
        )
        if template:
            for partner in self.production_id.mrp_production_follower_ids:
                self.send_email_to_follower(template, partner)
    ...
    def send_email_to_follower(self, template, partner):
        ...
        values = wizard._onchange_template_id(
            template.id, "mass_mail", "mrp.production.historical", self.id
        )["value"]
        wizard.write(values)
