...
const generateSerialsEntry = registry.category("view_widgets").get("generate_serials", null);
                ...
                class MOGenerateDialog extends DialogComponent {
                    setup() {
                        ...
                        onMounted(() => {
                            ...
                            if (nextSerial   && this.nextSerial?.el)      this.nextSerial.el.value      = nextSerial;
                            if (productQty !== undefined && this.nextSerialCount?.el) this.nextSerialCount.el.value = Math.floor(productQty);
