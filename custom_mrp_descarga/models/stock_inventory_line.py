        ...
        if self.inventory_id and self.inventory_id.batch_id:
            if self.prod_lot_id:
                entry_line = self.inventory_id.batch_id.move_line_ids.filtered(
                    lambda c: c.lot_id == self.prod_lot_id
                ...
                if entry_line and sum(entry_line.mapped("quantity")) != 0:
                    dev_line_amount = sum(dev_line.mapped("amount")) or 0
                    dev_line_qty = sum(dev_line.mapped("quantity")) or 0
                    if sum(entry_line.mapped("quantity")) - dev_line_qty != 0:
                        cost = (sum(entry_line.mapped("amount")) - dev_line_amount) / (
                            sum(entry_line.mapped("quantity")) - dev_line_qty
                        )
                    else:
                        cost = 0
                    self.cost = cost
