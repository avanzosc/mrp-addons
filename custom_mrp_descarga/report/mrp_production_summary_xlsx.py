    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        for line in movelines:
            if line.product_category_id not in categories:
                ...
                categ_lines = movelines.filtered(lambda c: c.product_category_id == line.product_category_id)
                ...
                categ_average_weight = round(sum(categ_lines.mapped("weight")),3) / len(categ_lines)
                categt_applied_price = round(sum(categ_lines.mapped("standard_price")),3) / len(categ_lines)
                ...
                for product in categ_lines:
                    if product.product_id not in products:
                        ...
                        product_average_weight = sum(
                            product_lines.mapped("weight")
                        ) / len(product_lines)
