...
class ChangeProductionQty(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if res.get("mo_id"):
            production = self.env["mrp.production"].browse(res["mo_id"])
            if production.state == "draft":
                raise UserError(_("You can not use the wizard to change quantity in draft manufacturing orders."))
    ...
    def _update_product_to_produce(self, production, qty, old_qty):
        return super()._update_product_to_produce(production, qty, old_qty)
