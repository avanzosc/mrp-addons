...
class StockScrap(models.Model):
    ...
    scrap_cost = fields.Float(string="Scrap Cost", digits="Product Price", copy=False)
    ...
    @api.onchange("price_unit_cost", "scrap_qty")
    def _onchange_price_unit_cost(self):
        self.scrap_cost = self.price_unit_cost * self.scrap_qty
    ...
    def create(self, vals_list):
        ...
        for scrap in scraps.filtered(lambda x: x.state == "done"):
    ...
    def write(self, vals):
        ...
        for scrap in self.filtered(lambda x: x.state == "done"):
            scrap.production_id.update_prodution_cost()
        return result
    ...
    def _calculate_price_unit_cost(self, vals):
        if "product_id" in vals:
            ...
            vals["price_unit_cost"] = product.standard_price
        if "lot_id" in vals and vals.get("lot_id", False):
            lot = self.env["stock.lot"].browse("lot_id")
            if lot.purchase_price:
                vals["price_unit_cost"] = lot.purchase_price
        if vals.get("price_unit_cost") and vals.get("scrap_qty"):
            vals["scrap_cost"] = vals.get("price_unit_cost") * vals.get("scrap_qty")
        return vals
